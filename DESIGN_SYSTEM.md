...
需要用户选择执行方式的操作使用单一 Secondary Menu Button，不拆成两个相邻热区。“打开全部”在大、中卡片和完整版显示完整文案；小卡片显示“打开”，但 `title` 与 `aria-label` 仍使用完整文案。

按钮和可打开的网站项在 pointer-down / `:active` 阶段给出轻微缩放反馈，不等待操作完成。该位移只使用 `transform`，时间不超过 100ms；`prefers-reduced-motion: reduce` 下移除缩放，保留颜色反馈。
